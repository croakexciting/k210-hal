...
impl APB2 {
    ...
    pub fn set_frequency(&mut self, expected_freq: impl Into<Hertz>) -> Hertz {
        let aclk = ACLK::steal();
        let aclk_freq = aclk.get_frequency().0 as i64;
        let apb2_clk_sel = (aclk_freq / expected_freq.into().0 as i64 - 1)
            .max(0).min(0b111) as u8;
        unsafe {
            sysctl().clk_sel0.modify(|_, w| w.apb2_clk_sel().bits(apb2_clk_sel));
        }
        Hertz(aclk_freq as u32 / (apb2_clk_sel as u32 + 1))
    }
